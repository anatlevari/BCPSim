...
def new_customer(tim, mu_r,isReturned):
    ...
    if queue.ninline >= b_size and random.random() <= pLeave:        
        rejected_till_now += 1
        targetfunction_val_vector.append(\
        np.exp(-1.0*alpha*tim)*(c_1*(min(queue.ninline,b_size) +(1-queue.nfreeserv)) +\
        ((c_2*rejected_till_now)/pLeave)))
        return
...
def calcCostOfB(mu_r, b, iteration):
    ...
    global targetfunction_time_vector
    global targetfunction_val_vector
    global eventvector_per_iter
    global timevector_per_iter
    global b_size
    global server_service_time
...
#        queue.place_last_in_line()
#    queue.call_next_in_line()
#    queue.put_event("Customer served", server_service_time.rexpo(1.0/mu_s, service_time_bound))
    rstrmarriv = CumulRandomStream(iteration*1984+1)  
    queue.put_event("New customer", rstrmarriv.rexpo_cum(lambda_new))
    targetfunction_time_vector = []
    targetfunction_val_vector = []
    rejected_till_now = 0
    # Actual simulation:
...
def calculateValOfMu(mu_r):
    ...
    for b in range(FIRST_B, MAX_B_SIZE+1, DELTA_B):
        xLabelsOfval += [str(b)]
        bToCost[(b-FIRST_B)/DELTA_B] = calcAvgCostOfB(mu_r,b)
